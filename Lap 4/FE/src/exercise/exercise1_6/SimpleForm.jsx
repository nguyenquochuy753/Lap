...
const SimpleForm = ({ children, value, onChange, onValid }) => {
	...
	const setValue = useCallback((field, v) => setValues((vs) => ({ ...vs, [field]: v })), [setValues]);

	const getValue = useCallback((field) => values[field], [values]);

	const setDirty = useCallback((field) => setDirtyFields((df) => ({ ...df, [field]: true })), [setDirtyFields]);

	const getDirty = useCallback((field) => Object.keys(dirtyFields).includes(field), [dirtyFields]);

	const setInvalid = useCallback(
		(field, error) => {
			setInvalidFields((i) => ({ ...i, [field]: error ? error : undefined }));
		},
		[setInvalidFields]
	);
